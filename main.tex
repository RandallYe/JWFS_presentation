...
\begin{frame}{Preferential choice $S \pref T$}
    \begin{itemize}
        ...
        \only<3->{\item New prospective values semantics,}
        \only<4->{\item Preference in wp: three-valued G\"{o}del logic~\cite{GoedelLogic} (false, true, supertrue), mechanised in Isabelle/HOL (wpe semantics of GCL)} 
...
%
    \only<2>{
    \begin{block}{wp}
        The closest one in wp is Nelson's biased choices~\cite{Nelson1989}: $ S \biasedchoice T$, but no continuation in biased choice. \par 
        Expect $\alert{S \choice T} \; \; \refby \; \; S \pref T \; \; \refby \; \; \alert{S \biasedchoice T}$ in wp, but $S \pref T$ cannot be in wp
    \end{block}
    }

    \only<3>{
    \begin{block}{Prospective values (pv) semantics}
       Based on expression transformers~\cite{bGSL2023}: $S \diamond E$, prospective values of expression $E$ after the executation of $S$\par
