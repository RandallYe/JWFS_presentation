...
\begin{frame}{Preferential choice $S \pref T$}
    \begin{itemize}
        ...
        \only<4->{\item Preference in wp \alert{\emph{is possible}} by adopting a three-valued G\"{o}del logic~\cite{GoedelLogic} (false, true, supertrue), mechanised in Isabelle/HOL (wpe semantics of GCL).
        \begin{itemize}
          \item Such a theory also caters for angelic choice.
        \end{itemize}}
...
%
    ...
    \note[item]<1>{$(x = 1 \precond \gclskip)$ is preconditioned program, equal to $\gclskip$ if $x=1$, otherwise aborts (non-termination)}
%
    \only<2>{
        \begin{block}{wp}
            The closest in wp is Nelson's biased choices~\cite{Nelson1989}: 
            \[ S \biasedchoice T \deq S \choice \lnot \fis(S) \guards T \]
            but no continuation in biased choice. 
            The following law holds: 
            \[ S \choice T \; \; \refby \; \; \alert{S \pref T} \; \; \refby \; \; S \biasedchoice T \]
            in a suitable (non-wp) semantic theory.
        \end{block}
    }
    \note[item]<2>{$b \guards P$ is guarded program, equal to $P$ if $b$, otherwise miracle or \nullbunch}
%
    \only<3>{
        \begin{block}{PV semantics}
            Based on expression transformers~\cite{bGSL2023}:
            \[ \alert{S \diamond E} \quad \mbox{(prospective values of expression $E$ after the execution of $S$)} \]
            Defined within Rick Hehner's bunch theory:
    ...
    \note[item]<3>{$\nullbunch$ - nothing, the contents in empty sets; $\bunchcomma$ - bunch union}
    \note[item]<3>{$\improper$ - improper bunch}
    \note[item]<3>{Why bunch theory? wp is not appropriate to give a semantics to preference. Bunch theory, based on its mechanics, allows us just to do that, and in a fairly intuitive and straightforward manner. So, bunch theory has been a prime candidate for a semantic model of preference; wpe came later}
    \note[item]<3>{Bunch theory solves undefined values, non-denoting terms, values for non-termination. $1/0 = \nullbunch$}
    \note[item]<3>{Definite description: $\mu x @ P$ what if there is no unique x to make $P$ established}
    \note[item]<3>{``Through use of bunches we have been able to capture interactions between probabilistic choice, non-deterministic choice, feasibility and aborting behaviour in the world of values.''~\cite{Stoddart2024}}
    \note[item]<4>{why no angelic choice? We played around with capturing angelic choice in our bunch-based pv semantics but realized that in order to do so, we require two different notions of bunch: demonic and angelic bunches of outcomes. The interplay between those is a point of complexity as well, including the underlying algebraic laws. My hunch is that it is possible but would require a more elaborate bunch theory with two different notions of a bunch that need to be syntactically delineated. (We already extended Rick Hehner’s original/classical bunch theory by included in improper bottom bunch to account for non-termination, so we did not want to add further complexity.) }
\end{frame}
...
%
    ...
    %\begin{block}{Probabilistic Unifying Relations (ProbURel)}
        \centering
        \includegraphics[scale=0.23]{pics/ProbURel.png}
        %\begin{align*}
            ...
            \begin{columns}
                ...
                \begin{column}{0.70\textwidth}
                    ...
                    \begin{align*}
                        ...
                        %
                        \only<3>{&\cle \ibracket{x > 0}*\ibracket{x'=0} * \\
                        &\left(
                        \begin{array}[]{l}
                            \only<3>{\cle \ibracket{t'-t\geq x}*\ibracket{((t'-t)-x)\%2=0}*\\
                                \cle \quad\mu(x-1, t'-t-1)*p +\\} 
                            \only<3>{\cle \ibracket{t'-t\geq x+2}*\ibracket{((t'-t)-(x+2))\%2=0}*\\
                        ...
                        &\left(
                        \begin{array}[]{l}
                            \only<4>{\cle \ibracket{t'-t\geq x}*\ibracket{((t'-t)-x)\%2=0}*\\
                                \cle \quad\mu(x-1, t'-t-1)*p +\\} 
                            \only<2,5->{\ibracket{t'-t\geq x}*\ibracket{((t'-t)-x)\%2=0}*\\
                                \quad\mu(x-1, t'-t-1)*p +\\} 
                            \only<2,4,6->{\ibracket{t'-t\geq x+2}*\ibracket{((t'-t)-(x+2))\%2=0}*\\
                                \quad\mu(x+1, t'-t-1)*q}
                            \only<5>{\cle \ibracket{t'-t\geq x+2}*\ibracket{((t'-t)-(x+2))\%2=0}*\\
                                \cle \quad\mu(x+1, t'-t-1)*q}
                        \end{array}\right) \\}
                        \only<2-5>{&\mu(m, n) = p*\mu(m-1,n-1) + q*\mu(m+1,n-1)}
                        \only<6>{&\cle \mu(m, n) = p*\mu(m-1,n-1) + q*\mu(m+1,n-1)}
                    \end{align*}
                \end{column}
            \end{columns}
        }
